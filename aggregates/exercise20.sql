...
SELECT name,
       CASE WHEN CLASS=1 THEN 'high' WHEN CLASS=2 THEN 'average' ELSE 'low' END revenue
FROM
  (SELECT facs.name AS name,
          ntile(3) OVER (
                         ORDER BY sum(CASE WHEN memid = 0 THEN slots * facs.guestcost ELSE slots * membercost END) DESC) AS CLASS
